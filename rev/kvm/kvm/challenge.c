...
extern OBF guest_tbl[];
extern int guest_tbl_len;


static OBF find_OBF(unsigned int a) {
  for (int i = 0; i < guest_tbl_len; ++i) {
    if (guest_tbl[i].sw == a)
      return guest_tbl[i];
  }
  fprintf(stderr, "Error - bug the organizer\n");
  exit(1);
}


static void run_vm(struct vm *vm, struct vcpu *vcpu) {
  ...
  for (;;) {
    ...
    case KVM_EXIT_IO:
      ...
      if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN &&
          vcpu->kvm_run->io.port == 0xE9) {
        char *p = (char *)vcpu->kvm_run;
        fread(p + vcpu->kvm_run->io.data_offset, vcpu->kvm_run->io.size, 1,
              stdin);
        continue;
      }
...
static void run_long_mode(struct vm *vm, struct vcpu *vcpu) {
